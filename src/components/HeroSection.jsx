...
const HeroSection = () => {
  ...
  const handleDownload = () => {
    ...
    link.href = "/CV_SIDIK WISNU SASMITO.pdf"; // Ganti dengan path file PDF kamu
  ...
  return (
    <div className='relative w-full'>
      <section className="w-full lg:h-[100vh] text-white px-4"
        id="home">
      ...
      '></header>
        <section className='relative -translate-y-10 w-full px-5 sm:px-8 md:px-12 lg:px-0 max-w-screen-lg lg:max-w-screen-xl mx-auto'>
          <article className=' flex flex-col lg:flex-row pt-15 lg:pt-0 lg:max-w-none max-w-2xl md:max-w-3xl mx-auto'>
            <section className=' lg:py-6 flex lg:w-max w-full justify-center items-center'>
              <SocialButtons />
              <section className=' md:ml-12 w-full'>
                <header className='text-center lg:text-left snap-center'>
                  <h1 className='pt-4 text-white font-bold text-2xl md:text-5xl lg:text-6xl'>
                    {"Hi, I'm "} <br />
            ...
            <figure className='justify-center items-center flex lg:flex-1 my-8 lg:pt-20 snap-center md:snap-none '>
              <motion.div
                className=' w-[300px] h-[300px] flex justify-center items-center p-3 rounded-full overflow-hidden relative'
                animate={{
                  backgroundColor: ['#51ff00', '#ff7556', '#ff6667'],
                  scale: [1, 1.1, 1],
                  rotate: [1, 5, 5, 1],
                }}
                transition={{
              ...
              >
                ...
                <img
                  src="/heroPic.jpg"
                  alt="Hero Pic"
                  className='relative z-10 rounded-full w-full h-full object-cover object-top shadow-2xl transform transition-transform duration-500 ease-in-out hover:scale-110 pointer-events-none'
                />
              </motion.div>
            </figure>
        ...
        <section className='mx-auto w-full relative text-white flex justify-center -translate-y-5 snap-center'>
          <header className='absolute w-1/2 aspect-[16/5] -skew-x-12 rounded-full bg-gradient-to-r from-blue-600 via-blue-800 to-amber-200 opacity-20 blur-[100px] left-10 top-0  md:block
                '></header>
          <header className='absolute w-1/2 aspect-[16/5] -skew-x-12 rounded-full bg-gradient-to-r from-blue-600 via-blue-800 to-amber-200 opacity-20 blur-[100px] right-10 bottom-10  md:block
                '></header>
          ...
          <motion.section
            ...
            className='relative z-1 mx-auto w-11/12 lg:mx-0 p-5 sm:p-6 py-6 sm:py-8 rounded-3xl border-amber-300 bg-[#ffffff29] backdrop-blur-xs shadow-lg md:divide-x divide-amber-300 grid grid-cols-2 md:grid-cols-4 gap-4 gap-y-6 md:gap-8 lg:gap-12'
          >
            {Number.map((item) => (
              <article key={item.id} className='text-center'>
                ...
                <p className='text-lg mt-1 md:mt-2 '>{item.title}</p>
              </article>
            ))}
          </motion.section>
        </section>
